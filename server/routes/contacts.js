...
router.patch('/:id', async (req, res) => {
  ...
  try {
    // CRM-matched contacts are read-only for name edits — the CRM is the source
    // of truth and local renames would silently diverge from it. Contacts with
    // no CRM match (random numbers texting in) stay fully editable.
    if (name !== undefined) {
      const { rows: [existing] } = await pool.query(
        'SELECT id, phone_number, zoho_contact_id FROM contacts WHERE id = $1', [req.params.id]
      );
      ...
      if (!crmMatched && process.env.ZOHO_REFRESH_TOKEN) {
        try {
          ...
          if (zc) {
            crmMatched = true;
            // Cache the match so future checks skip the API round-trip
            await pool.query(
              'UPDATE contacts SET zoho_contact_id = $1, zoho_synced_at = NOW() WHERE id = $2',
              [zc.id, existing.id]
            );
          }
        } catch (e) { /* Zoho unreachable — fail open, allow the edit */ }
      }
      if (crmMatched) {
        return res.status(409).json({ error: 'This contact is managed in the CRM — edit it there.', crm_matched: true });
      }
    }
    const { rows } = await pool.query(
      `UPDATE contacts
    ...
    if (!rows[0]) return res.status(404).json({ error: 'Contact not found' });
    // Let all clients refresh conversation lists so the new name shows everywhere
    try {
      const { getIO } = require('../socket');
      getIO()?.emit('conversation_updated', { contact_id: rows[0].id });
    } catch (e) { /* socket not initialized (tests) — fine */ }
    res.json(rows[0]);
  } catch (e) {
