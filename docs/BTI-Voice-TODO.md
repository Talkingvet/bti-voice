...
2. [ ] **iOS bugs found on first TestFlight install, 2026-08-20** (in priority order):
   ...
   - [ ] **Recording player controls dead on iOS.** Duration/timestamp now render (Content-Length fix worked) but play, speaker and scrub do nothing. Cause: `CallsTab.jsx` uses a native `<audio controls>`; iOS renders those via shadow-DOM controls that are unreliable in a Capacitor WKWebView with the viewport locked (`user-scalable=no`, `scrollEnabled:false`). **Fix: replace with a custom player** — own play/pause button, `<input type=range>` scrubber driven by `timeupdate`. Two sites: the call-detail player (~line 410) and the voicemail mini-player (~line 476).
   ...
   - [ ] **Connection dot permanently red on iOS** even though calls work. Twilio device is clearly registering; the TitleBar indicator logic is misreading state on iOS. Per `0be6d33` the dot is only supposed to appear when degraded.
3. [ ] **Promote the day-to-day ASC account from Customer Support → Admin** so Account Holder credentials aren't needed for routine TestFlight work.
